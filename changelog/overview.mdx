...
AllKnower now routes any LLM or embedding task to a local OpenAI-compatible server (Ollama, LM Studio, and similar). Prefix the model ID with `ollama/` or `local/` to route that task locally, and configure your server with `LOCAL_PROVIDER_BASE_URL` and `LOCAL_PROVIDER_API_KEY`. Mix local and OpenRouter entries in the same fallback chain — if a local model fails, AllKnower tries the next entry, even when it is a cloud model. See [Configuration reference](/self-hosting/configuration) for setup.
