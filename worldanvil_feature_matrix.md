...
- first-class hierarchy and publishing UX in the Portal
- maps, timelines, and calendars
- campaign/session tooling
- collaboration and audience segmentation
- system-content packaging such as SRDs, rulesets, and VTT integrations
...
| Categories / folder-like hierarchy / nested sub-categories | Already have - half-assed | The data model supports true tree branches, but the Portal sidebar currently behaves mainly as a lore-type/category filter rather than a full nested content tree. | Build a real branch-tree browser and drag-reparent UI on top of the existing branch APIs. | M |
| Tags | Already have - half-assed | Labels already behave like tags, but tag authoring and tag-driven browsing are not first-class Portal UX. | Add explicit tag chips, tag editing, and tag filters. | S-M |
...
| Broad template coverage | Already have - half-assed | Portal exposes a useful set, but backend types/templates support more than the Portal picker currently surfaces. | Expose organization, race, myth, cosmology, deity, religion, and other already-supported types. | S-M |
...
| Public wiki / publication surface | Already have - half-assed | Core public share pages are powerful, but Portal does not present them as a polished publishing workflow. | Add publish/share management UI in Portal. | M |
| Per-entry public / private toggle | Already have - half-assed | Draft and `gmOnly` mechanics exist, but Portal does not expose a polished share-state control per note. | Add explicit share-state controls and share-tree placement UI. | M |
| Advanced privacy settings | Already have - half-assed | Core supports passworded shares, `gmOnly`, `draft`, `shareIndex`, and related share labels. Portal barely surfaces this. | Build a share settings panel in Portal. | M |
...
| Visual timeline | Can have | Timeline entities/templates exist in backend types, but Portal has no visual timeline UI. | Build a timeline view for event and timeline notes. | M |
| Simple list-mode timeline | Can have | Same data model gap as above. | Ship a list view first if speed matters. | S-M |
...
| World Anvil feature | Current verdict | What we have now | How to reach parity or exceed it | Effort |
|---|---|---|---|---|
| Campaign planner | Can have | No campaign entity or planner UI exists. | Add campaign/session models and planner pages. | L |
| Session planner / session organizer | Can have | No first-class session planning flow exists. | Add session notes plus planner UI. | M-L |
| Session reports | Can have | Brain Dump can summarize raw notes, but there is no dedicated report model or session archive. | Add session report notes and recap flows. | M |
| Digital DM screen | Can have | Nothing equivalent exists as a Portal surface. | Build a DM workspace with pinned notes, search, quick links, and actions. | L |
| Create NPCs during play | Already have - half-assed | New note creation and Brain Dump make this possible, but not optimized for live sessions. | Add quick-create inside a future DM screen. | M |
| Statblock library | Already have - half-assed | Statblock type exists, but there is no dedicated library or session picker. | Add statblock browsing, filters, and references. | M |
| Homebrew statblocks | Already have - half-assed | Backend types and templates support statblocks, but Portal treatment is minimal. | Build dedicated statblock editing and display UIs. | M |
...
| Search and pull statblocks during a session | Can have | Depends on statblock library + DM screen. | Add session-side search and quick insert/reference. | M |
...
| D&D 5e SRD | Can have | No official SRD ingestion or browser exists. | Import SRD content and build browse/search views. | L |
| Pathfinder / PF2 support | Can have | Same as above. | Build system packs or importers. | L |
| Dozens of systems support | Already have - half-assed | The statblock schema already includes a `system` field, but there is no actual system library infrastructure. | Create a generic system-pack architecture. | L |
...
|---|---|---|---|---|
| P0 | Brain Dump `POST` response contract is drifting | The AllKnower Brain Dump pipeline returns `created` and `updated` arrays, while the Portal types/results expect `notesCreated`, `notesUpdated`, and `entities`. This is fragile and likely already wrong in parts of the UI. | Define one shared Brain Dump response contract and normalize it in the Portal proxy. Add contract tests between Portal and AllKnower. | S |
| P0 | Brain Dump history contract is wrong today | AllKnower history currently returns note ID arrays and no summary/entity detail, while the Portal history UI expects counts, summary, and entities. This is a real bug, not just a roadmap gap. | Either enrich `GET /brain-dump/history` or add a history-detail route and transform the data consistently in the Portal. Add shared schemas/tests. | S-M |
...
| P1 | The Portal docs overstate hierarchy parity | The existing high-level gap doc describes a shipped hierarchical tree, but the live Portal sidebar is still closer to a lore-type filter than a real nested branch browser. | Build a genuine branch-tree UI and update docs to reflect actual shipped behavior. | M |
| P1 | Share and publish capabilities are underexposed | AllCodex Core already has strong share primitives such as `shareRoot`, passworded shares, `gmOnly`, `draft`, `shareIndex`, `shareRaw`, and share aliases. The Portal barely exposes them. | Surface share-state, publishing, and audience settings in the Portal before building entirely new share systems. | M |
| P1 | The backend already supports more lore types than the Portal exposes | Timeline, organization, race, myth, cosmology, deity, religion, and other types exist in backend schemas/templates, but the Portal picker does not expose them cleanly. | Expand the Portal type picker and browsing UX before inventing new data models. | S-M |
