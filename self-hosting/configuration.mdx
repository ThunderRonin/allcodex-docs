...
Mix local and cloud models freely within a fallback chain. AllKnower detects the prefix per model and strips it before calling your local server. It also prunes OpenRouter-specific parameters (provider routing, reasoning effort, trace headers) so the local endpoint receives a clean OpenAI-compatible request. If a local model errors or times out, AllKnower tries the next entry in the chain — even when that entry is a cloud model.
